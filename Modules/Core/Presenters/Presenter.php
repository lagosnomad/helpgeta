...
abstract class Presenter
{
    ...
    public function thumbSrc($width = null, $height = null, array $options = array(), $field = 'image')
    {
        ...
        if ($width || $height) {
            $src = Croppa::url(url($src), $width, $height, $options);
        }
        return $src;
    }
    ...
    public function body()
    {
        ...
        if (is_array($matches)) {
            foreach ($matches as $match) {
                $patterns[] = $match[0];
                $module = $match[1];
                $repository = app('Modules\\'.ucfirst(str_plural($module)).'\Repositories\\'.ucfirst($module).'Interface');
                $model = $repository->byId($match[2]);
                if ($module == 'page') {
                    $replacements[] = url($model->uri);
                } else {
                    $replacements[] = route($module.'.slug', $model->slug);
                }
            }
        }
